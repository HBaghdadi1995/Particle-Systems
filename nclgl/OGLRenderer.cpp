...
GlShader*		   OGLRenderer::debugDrawShader			= NULL;
...
OGLRenderer::OGLRenderer(Window &window)	{
	...
	if(!debugDrawingRenderer) {
		debugDrawShader		 = new GlShader(SHADERDIR"/DebugVertex.glsl", SHADERDIR"DebugFragment.glsl");
...
void OGLRenderer::SetCurrentShader(GlShader*s) {
	currentShader = s;
